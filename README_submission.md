...
Baseline no-memory dat 2/11 (18,18%): chi E01 va E10 short-term pass; long-term, episodic, semantic va mixed deu 0%. Student retrieval chua the chay cloud tren may nay vi chua co `ZEP_API_KEY`, nen toi khong ghi mot hit rate gia. E07 du kien retrieve nhieu token nhat vi hop nhat long-term preference va semantic payment rule. Case nay bat buoc co ca `Python` va `Idempotency-Key`.
...
E08 minh hoa recency theo scope: `BLUEBIRD-42` dung TypeScript/NestJS, trong khi Python van hop le cho demo ca nhan `ORCHID-27`; khong nen xoa fact cu khoi scope khac. E10 cho thay compaction phai giu durable constraint `REVIEW-DEADLINE-1600`, Friday 16:00 ngay ca khi raw turn da bi evict. Buffer don thuan tang token tuyen tinh va khong tao durable note.
...
De chong memory poisoning, durable write can opt-in, user-scoped namespace, allowlist type, source/timestamp/confidence, PII minimization va review cho preference/task tac dong cao. Retrieval uu tien fact moi dung scope, van giu provenance cua fact bi supersede. Heartbeat chi duoc deduplicate/expire/recap, khong tu cap quyen hay tao instruction moi.
